<template>
    <div class="section" style="margin-top: -4.67vh;">
        <div class="container input-group">
            <input type="text" v-model="search"  class="form-control border border-danger p-3" placeholder="在码农之路中利用回车搜索..." style="z-index: 1;width: 100%;height: 86.766px;color: #92a0aa;box-shadow: rgba(0,0,0,.2) 0px 5px 5px 0.5px;">
            <div class="input-group-append">
                <button class="btn position-relative mr-1 d-xl-block d-none d-xl-block d-none" type="submit" @click="searchTitle" style="margin-top: -6.48vh;margin-left: 979.6px;width: 40px;height: 40px;background-image: url(/images/fas\ fa-search\ red.svg);background-size: 40px 40px; background-color: transparent;background-position: center;background-repeat: no-repeat;"></button>
                <a href="#" class="btn position-relative mx-4 d-xl-block d-none" style="margin-top: -6.48vh;width: 40px;height: 40px;background-image: url(/images/fas\ fa-plus.svg);background-size: 40px 40px; background-color: transparent;background-position: center;background-repeat: no-repeat;"></a>
            </div>
        </div>
        <div class="container mt-4 mb-4">
            <div class="row row-cols-1 row-cols-sm-2 row-cols-lg-3">
                <div class="col" v-for="data in artDatasMark" :key="data._id">
                    <div class="card mb-4 text-danger card-shadow">
                        <a style="cursor:pointer;">
                            <img class="card-img-top" :src="data.base64" @click="handleToDetail(data._id)" style="width: 100%;height: 100%;">
                        </a>
                        <div class="card-body py-3">
                            <a style="text-decoration: none;cursor:pointer;">
                                <h5 class="card-title pb-3 border-bottom border-secondary text-danger text-truncate" @click="handleToDetail(data._id)">{{data.Title}}</h5>
                            </a>
                            <div class="float-left mt-1">
                                <img src="/images/far fa-clock red.svg" class="clock" style="width: 18px;height: 18px;margin-bottom: 4px;">
                                <span class="ml-1">{{data.Time}}</span>
                            </div>
                            <a class="card-type float-right mt-1 text-decoration-danger" style="cursor:pointer;" @click="handleToType()">
                                <img src="/images/fas fa-tags Copy red.svg" class="tag" style="width: 18px;height: 18px;margin-bottom: 2px;">
                                <span class="card-type text-danger ml-1">{{data.Type}}</span>
                            </a>
                        </div>
                    </div>
                </div>
            ...
            <div class="pageObjs mb-4">
                <div class="container">
                    <div class="row row-cols-3">
                        ...
                        <p class="col-8 text-center mb-0 align-self-center text-danger" id="page" style="font-size: 2.25rem;">
                            {{(this.page+12)/12}}  /  {{Math.ceil(artDatas.length/12)}}
                        </p>
                        <div class="col-2">
                            <button type="button" class="btn-radius btn-radius-alive float-right border-0 page-right" style="width: 64px;height: 64px;background-image: url(/images/fas\ fa-chevron-circle-right\ red.svg);background-position: center;background-repeat: no-repeat;background-clip: content-box;"
...
export default {
    ...
    mounted(){
        axios.get("/api/profiles/码农之路").then(res => {
